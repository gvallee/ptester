...
# Print a configuration once the configuration is loaded.
# @parma[in]    Reference to the hash representing the configuration
sub print_config ($)
{
	my ($ref) = @_;
...
# @param[in]    Absolute path to the configuration file to parse and load.
# @return       Reference to a hash representing the configuration (opaque handle)
sub load_config ($)
{
	my ($configfile) = @_;
