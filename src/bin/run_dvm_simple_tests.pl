...
if ($help)
{
	print "Usage: $0 --outputdir <DIR> --project <LSFPROJECTID> --platform <PLATFORM> [--queue <LSF_QUEUE>] [--resultfile] [--verbose] [--help]\n";
	...
	print "\t--platform     Execution platform's name.\n";
	print "\t--queue        LSF queue to use to submit jobs for testing.\n";
	print "\t--resultsfile  Use the result file generated by the job rather than the output of the job to determine success.\n";
...
do
{
	...
	$np = _get_np ($nnodes);
	$cmd = "cd $output_dir; $binDir/run_dvm_simple_test.pl --project $project --configlog $configlog --nnodes $nnodes --np $np --output-dir $output_dir";
